...
test3(void *x)
{
  ...
  //printf ("**\n%u test3: client %d acquire a release a concurrent\n**\n",(unsigned int)pthread_self(), i);
  for (int j = 0; j < 10; j++) {
    //printf ("**\n%u test3: client %d going to get lock\n", (unsigned int)pthread_self(), i);
    //ofile << "**"<<(unsigned int)pthread_self()<<lc[i]->id<<" test3: client "<<i<<" going to get lock\n" << std::endl;
    lc[i]->acquire(a);
    check_grant(a);
    printf ("**\n%u test3: client %d got lock\n", (unsigned int)pthread_self(), i);
    check_release(a);
    //printf ("**\n%u test3: client %d going to release lock\n**\n", (unsigned int)pthread_self(), i);
    lc[i]->release(a);
    printf ("**\n%u test3: client %d release lock\n**\n", (unsigned int)pthread_self(), i);
  }
  //printf("******%ul finish*******\n", (unsigned int)pthread_self());
  //ofile << (unsigned int)pthread_self() << " finished" << std::endl;
  //++mycnt;
  //printf("*****cnt %i*****", mycnt);
  return 0;
}
...
test5(void *x)
{
  ...
  for (int j = 0; j < 10; j++) {
    //printf ("test5: pthread %u client %u going to got lock\n",(unsigned int)pthread_self(), i);
    //ofile << (unsigned int)pthread_self() <<" client" << i  << " going to get lock" << std::endl;
    if (i < 5)  lc[0]->acquire(a);
    else  lc[1]->acquire(a);
    check_grant(a);
    //printf ("test5: pthread %u client %d got lock\n",(unsigned int)pthread_self(), i);
    //ofile << (unsigned int)pthread_self() <<" client" << i  << " get lock" << std::endl;
    check_release(a);
    if (i < 5) lc[0]->release(a);
    else lc[1]->release(a);
    //printf ("test5: pthread %u client %d release lock\n",(unsigned int)pthread_self(), i);
  }
  //printf ("test5: pthread %u finish\n",(unsigned int)pthread_self());
  //ofile << (unsigned int)pthread_self() << " finished" << std::endl;
  return 0;
}
...
main(int argc, char *argv[])
{
    ...
    ofile.open("tester.log",std::ios::out);
    if(!test || test == 3){
      printf("test 3\n");
